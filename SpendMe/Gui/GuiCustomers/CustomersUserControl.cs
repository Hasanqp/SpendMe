...
namespace SpendMe.Gui.GuiCustomers
{
    public partial class CustomersUserControl : UserControl
    {
                        ...
                        for (int i = 0; i < IdList.Count; i++)
                        {
                            ...
                            if (result == 1)
                            {
                                ...
                                SystemRecords systemRecords = new SystemRecords
                                {
                                    ...
                                    Details = "Клиент с идентификационным номером был удален №" + RowId.ToString(),
        ...
        public async void LoadData()
        {
            loadingForm.Show();
            var data = await dataHelper.GetAllDataAsync();
            var ListOfCutomerId=data.Select(x=>x.Id).ToList();
            await Task.Run(() => UpdateData(ListOfCutomerId));
        ...
        private void UpdateData(List<int> LIstOfCustomersId)
        {
            ...
            {
                var CustomerId = LIstOfCustomersId[i];
                try
                {
